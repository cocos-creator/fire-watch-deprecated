...
describe('FireWatch Simple Case', function () {
  ...
  it('should work for rename file', function ( done ) {
    ...
    var watcher = FireWatch.start( root, function () {
      ...
      Fs.renameSync(
        Path.join(root, 'foo/foo-02/foobar.js'),
        Path.join(root, 'bar/bar-02/foobar-rename.js')
      );

      Fs.renameSync(
        Path.join(root, 'foo/foo-03/foobar.js'),
        Path.join(root, 'bar/bar-02/foobar-rename.js')
      );
...
describe('FireWatch Error Handling', function () {
  ...
  it('should report unknown-changed for file operations happen between the watcher start and started', function ( done ) {
    this.timeout(10000);
    ...
    watcher.on( 'unknown-changed', function ( results ) {
      expect(results).to.eql([
        Path.join(root, 'foobar.js')
      ]);
    });
